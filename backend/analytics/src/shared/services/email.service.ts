...
export class EmailService {
  ...
  async sendPasswordResetEmail(email: string, token: string): Promise<void> {
    try {
      ...
      await this.transporter.sendMail({
        from: this.configService.get<string>('SMTP_FROM', 'noreply@superapp-gamifier.com'),
        to: email,
  ...
  async sendWelcomeEmail(email: string, data: { name: string; firstStep: { title: string; description: string } }): Promise<void> {
    try {
  ...
  async sendOnboardingCompletionEmail(email: string, data: { name: string }): Promise<void> {
    try {
      const dashboardLink = `${this.frontendUrl}/dashboard`;
