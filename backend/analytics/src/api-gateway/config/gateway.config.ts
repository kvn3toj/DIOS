...
export default registerAs('gateway', () => ({
  ...
  cache: {
    ...
    ttl: parseInt(process.env.GATEWAY_CACHE_TTL, 10) || 300, // 5 minutes
  ...
  circuitBreaker: {
    ...
    resetTimeout: parseInt(process.env.CIRCUIT_BREAKER_RESET_TIMEOUT, 10) || 30000 // 30 seconds
  },
  cors: {
  ...
  security: {
    rateLimit: {
      ...
      windowMs: parseInt(process.env.SECURITY_RATE_LIMIT_WINDOW_MS, 10) || 900000, // 15 minutes
  ...
  monitoring: {
    metrics: {
      ...
      interval: parseInt(process.env.MONITORING_METRICS_INTERVAL, 10) || 60000 // 1 minute
    },
    healthCheck: {
      enabled: process.env.MONITORING_HEALTH_CHECK_ENABLED === 'true',
      interval: parseInt(process.env.MONITORING_HEALTH_CHECK_INTERVAL, 10) || 30000 // 30 seconds
    }
  },
  discovery: {
    enabled: process.env.SERVICE_DISCOVERY_ENABLED === 'true',
    provider: process.env.SERVICE_DISCOVERY_PROVIDER || 'consul',
    refreshInterval: parseInt(process.env.SERVICE_DISCOVERY_REFRESH_INTERVAL, 10) || 60000 // 1 minute
  },
  loadBalancing: {
    ...
    healthCheck: {
      ...
      interval: parseInt(process.env.LOAD_BALANCING_HEALTH_CHECK_INTERVAL, 10) || 10000 // 10 seconds
    }
  },
  transformation: {
    request: {
